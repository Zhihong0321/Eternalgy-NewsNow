...
```bash
# Fetch PV Magazine news
curl "http://localhost:3000/api/s?id=pvmagazine"

# Fetch The Star Online news
curl "http://localhost:3000/api/s?id=thestar"

# Force fetch latest (requires authentication)
curl "http://localhost:3000/api/s?id=cleantechnica&latest=true" \
...
async function fetchAllNewsSources() {
  ...
  const response = await fetch('http://localhost:3000/api/s/entire', {
    method: 'POST',
    headers: {
...
def fetch_all_news_sources():
    ...
    response = requests.post(
        'http://localhost:3000/api/s/entire',
